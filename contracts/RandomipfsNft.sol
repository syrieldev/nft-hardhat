...
contract RandomIpfsNft is ERC721URIStorage, VRFConsumerBaseV2, Ownable {
    ...
    // VRF Helpers
    // so that we can map users who requested mint and their chainlink random result
    mapping(uint256 => address) public s_requestIdToSender;

    // Events
    event NftRequested(uint256 indexed requestId, address requester);
    event NftMinted(Cats cat, address minter);
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        Cats catType = getTypeFromModdedRng(moddedRng);
    ...
    function getTypeFromModdedRng(uint256 moddedRng) public pure returns (Cats) {
        uint256 cumulativeSum = 0;
        uint256[3] memory chanceArray = getChanceArray();
        for (uint256 i = 0; i < chanceArray.length; i++) {
            // cat = 0 - 9  (10%)
            // kitten = 10 - 39  (30%)
            // silhouetteCat = 40 = 99 (60%)
    ...
    function transferOwnership(address newOwner) public override onlyOwner {
        ...
        require(newOwner != s_owner, "New owner address must be different from the current owner");
        _transferOwnership(newOwner); // Call the _transferOwnership function from the Ownable contract
        s_owner = newOwner;
    }
